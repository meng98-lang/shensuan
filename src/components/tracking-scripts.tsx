...
export default async function TrackingScripts() {
  ...
  return (
    <>
      {pixels.googleAnalytics && (
        <script async src={`https://www.googletagmanager.com/gtag/js?id=${pixels.googleAnalytics}`} />
      )}
      {pixels.googleAnalytics && (
        <script dangerouslySetInnerHTML={{
          __html: `
            ...
            gtag('js', new Date());
            gtag('config', '${pixels.googleAnalytics}');
          `
        }} />
      )}
      ...
      {pixels.customCode && (
        <script dangerouslySetInnerHTML={{ __html: pixels.customCode }} />
      )}
    </>
  );
}
